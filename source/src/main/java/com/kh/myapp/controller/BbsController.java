...
//	}
	...
	@GetMapping({ "/list", "/list/{reqPage}", "/list/{reqPage}/{searchType}",
	...
	/// 게시글 읽기
	@GetMapping({ "/read/{page}/{bnum}", "/read/{page}/{bnum}/{searchType}/{keyword}" })
	public String read(@PathVariable String bnum, @PathVariable String page,
			@PathVariable(required = false) String searchType, @PathVariable(required = false) String keyword,
			Model model) {
	...
	public String modify(@PathVariable String bnum, @PathVariable String page, Model model) {
	...
	public String modify(@Valid @ModelAttribute("bbsDTO") BbsDTO bbsDTO, BindingResult result,
	...
	/// 답글 처리
	@PostMapping("/reply")
	public String reply(@Valid @ModelAttribute("bbsDTO") BbsDTO bbsDTO, BindingResult result, Model model) {
		logger.info("replyForm 호출됨!");
